...
exports.getMessagesBySchool = async (req, res) =>{
    ...
    if(user._schools.includes(schoolId)){
        const messages = await (await School.findById(schoolId)).populated('_messages')
...
exports.getMessagesByApplicant = async (req, res) => {
    const {_id} = req.user
    const user = await User.findById(_id).populate('_applications')
    const {_messages} = user
    res.status(201).json(_messages)
}

exports.getMessagesBySchool = async (req, res) => {
    const {schoolId} = req.params
    const {_id} = req.user
    const school = await School.findById(schoolId).populate('_messages')
    const user = await User.findById(_id)
    if(user._schools.includes(schoolId)){
        res.status(201).json(school._messages)
    } else {
        res.status(401).json({message: 'Unathorized'})
    }
}
